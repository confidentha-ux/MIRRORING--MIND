...
export default function App() {
  ...
  if (journey.screen === "welcome") {
    return <WelcomeScreen hasSavedJourney={Boolean(savedJourney)} onBegin={() => updateJourney({ screen: "map" })} onResume={() => savedJourney && setJourney(savedJourney)} />;
  }

  if (journey.screen === "map") {
    return <QuestMap quests={quests} unlockedIndex={journey.unlockedIndex} onSelect={openQuest} onReset={resetJourney} />;
  }

  if (journey.screen === "intro") {
    return <QuestIntro quest={activeQuest} quests={quests} activeIndex={journey.questIndex} onStart={() => updateJourney({ screen: "question", questionIndex: 0 })} onMap={() => updateJourney({ screen: "map" })} />;
  }

  if (journey.screen === "question") {
    return <QuestionCard quest={activeQuest} quests={quests} activeIndex={journey.questIndex} question={activeQuestion} questionIndex={journey.questionIndex} questionCount={activeQuest.questions.length} value={currentValue} onChange={updateResponse} onBack={goBackFromQuestion} onContinue={continueQuestion} />;
  }

  if (journey.screen === "stageReflection") {
    return <StageReflection quest={activeQuest} quests={quests} activeIndex={journey.questIndex} responses={journey.responses} onContinue={() => updateJourney({ screen: "reward" })} onMap={() => updateJourney({ screen: "map" })} />;
  }

  if (journey.screen === "reward") {
    return <RewardCard quest={activeQuest} quests={quests} activeIndex={journey.questIndex} isFinalQuest={journey.questIndex === quests.length - 1} onContinue={continueFromReward} onMap={() => updateJourney({ screen: "map" })} />;
  }

  return <ReflectionCard reflection={reflection} onRestart={resetJourney} onMap={() => updateJourney({ screen: "map" })} />;
}
